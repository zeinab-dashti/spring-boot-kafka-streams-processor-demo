...
public class ApiController {
    ...
    public void addActivity(@RequestBody UserActivity userActivity) {
        userActivityProducer
                .addActivity(userActivity.getActivity())
                .whenComplete((result, ex) -> {
                    if (ex == null) {
                        log.info("User activity added: {}", result.getProducerRecord().value());
                    } else {
    ...
    public void addIotStatus(@RequestBody IotStatus iotStatus) {
        iotStatusProducer
                .addStatus(iotStatus.getStatus())
                .whenComplete((result, ex) -> {
                    if (ex == null) {
                        log.info("IOT status added: {}", result.getProducerRecord().value());
                    } else {
