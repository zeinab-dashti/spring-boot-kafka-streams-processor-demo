...
class IotStatusInputPipelineIT {
    ...
    public void setUp() {
        ...
        consumer.subscribe(Collections.singletonList(KafkaConfig.IOT_OUTPUT_TOPIC));

        // Kafka Streams configuration
        Properties streamsProps = KafkaConfig.setup("iot-status-app", "state/iot-status");
        streamsProps.put(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, KafkaTestContainerSetup.getBootstrapServers());

        Topology topology = IotStatusInputPipeline.buildIotTopology();
    ...
    public void testSessionExpiration() {
        ...
        // Send initial record
        producer.send(new ProducerRecord<>(KafkaConfig.IOT_INPUT_TOPIC, key, value));

        // Consume and verify the output
        ConsumerRecord<String, String> record = consumer.poll(Duration.ofSeconds(10)).iterator().next();
        assertEquals("Anomaly detected: " + value, record.value());
    }
}